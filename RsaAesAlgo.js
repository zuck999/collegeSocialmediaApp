...
class AES {
  constructor() {
    this.sBox = [ // h = 104 , hexa = 0x68  = Row 6 |, Column 8--.
    ...
    this.invSBox = [//0x45 = Row 4 |, Column 5--. = 0x68
    ...
    this.rCon = [//rCon = Round Constant. fixed table of numbers used in the AES Key Expansion process
  ...
  invMixColumns(state) {
    for (let i = 0; i < 4; i++) {
      ...
      state[0][i] = this.gmul(s0, 14) ^ this.gmul(s1, 11) ^ this.gmul(s2, 13) ^ this.gmul(s3, 9);
      state[1][i] = this.gmul(s0, 9) ^ this.gmul(s1, 14) ^ this.gmul(s2, 11) ^ this.gmul(s3, 13);
      state[2][i] = this.gmul(s0, 13) ^ this.gmul(s1, 9) ^ this.gmul(s2, 14) ^ this.gmul(s3, 11);
      state[3][i] = this.gmul(s0, 11) ^ this.gmul(s1, 13) ^ this.gmul(s2, 9) ^ this.gmul(s3, 14);
    }
  }

  // Encrypt a single 16-byte block
  encryptBlock(block, key) { //block>> [16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16]
    const state = [
    ...
    for (let round = 1; round < 10; round++) {
      ...
      const roundKey = [//add my key it adds 4 words in total there is 44 in 10 round
        [w[round*4][0], w[round*4+1][0], w[round*4+2][0], w[round*4+3][0]],
        [w[round*4][1], w[round*4+1][1], w[round*4+2][1], w[round*4+3][1]],
        [w[round*4][2], w[round*4+1][2], w[round*4+2][2], w[round*4+3][2]],
        [w[round*4][3], w[round*4+1][3], w[round*4+2][3], w[round*4+3][3]]
      ];
...
//no mix colum
  ...
  encrypt(message, key) {  // step 1
    ...
    const paddedMessage = Buffer.concat([messageBytes, Buffer.alloc(paddingLength, paddingLength)]);//PKCS#7 ppadding .. we can give only one which add only 00 by giving 2nd arg as the lenghth it will add same number of padding like hello remain 11 it add 0b which is 11
    ...
    const encrypted = [];
    for (let i = 0; i < paddedMessage.length; i += 16) {// it first encode 1 block then another then combile all block
...
class HybridCrypto {
  ...
  decrypt(encryptedData, rsaPrivateKey) {
    ...
    const encryptedMessage = Buffer.from(encryptedData.encryptedMessage, 'base64');
...
console.log(`   Public Key (e, n): (${publicKey.e}, ${publicKey.n.toString().slice(0, 30)}...)`);
console.log(`   Private Key (d, n): (${privateKey.d.toString().slice(0, 30)}..., ${privateKey.n.toString().slice(0, 30)}...)`);

// Message to encrypt
const message = 'Hello! This is a secret message for my bachelor project. AES and RSA working together!';
console.log(`\n[2] Original Message:\n   "${message}"`);
...
console.log(`   Encrypted AES Key (RSA): ${encrypted.encryptedKey.slice(0, 50)}...`);
console.log(`   Encrypted Message (AES): ${encrypted.encryptedMessage.slice(0, 50)}...`);
